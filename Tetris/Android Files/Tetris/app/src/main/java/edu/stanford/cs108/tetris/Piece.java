...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		skirt = new int[maxWidth+1];
		Arrays.fill(skirt, maxHeight);
		for(TPoint point : points) {
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] nextPoints = new TPoint[body.length];
		int idx = 0;
		for(TPoint point : body) {
			int x = point.x;
			int y = point.y;
			int nextX = -y+height-1;
			int nextY = x;
			nextPoints[idx++] = new TPoint(nextX, nextY);
		}
		Piece nextPiece = new Piece (nextPoints);
